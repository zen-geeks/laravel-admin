...
<li class="nav-item">@if(url()->isValidUrl($item['uri']))<a href="{{ $item['uri'] }}" target="_blank" class="nav-link">@else<a href="{{ admin_url($item['uri']) }}" class="nav-link">@endif<i class="nav-icon fas {{ $item['icon'] }}"></i><p>{{ $title }}</p></a></li>
    @else
