...
class StepForm extends Form
{
    ...
    protected function addFooter()
    {
        ...
        if ($index !== 0) {
            ...
            $footer .= "<a href=\"{$prevUrl}\" class=\"btn btn-warning float-start\">{$trans['prev']}</a>";
        }

        if ($index !== count($this->steps) - 1) {
            $footer .= "<button class=\"btn btn-info float-end\">{$trans['next']}</button>";
        }

        if ($index === count($this->steps) - 1) {
            $footer .= "<button class=\"btn btn-info float-end\">{$trans['submit']}</button>";
        }
